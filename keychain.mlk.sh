	...
	if [ "$inheritwhich" != none ]; then
		...
		if wantagent gpg; then
			if [ -n "$GPG_AGENT_INFO" ]; then
				...
				echo "inherit_gpg_agent_info = $inherit_gpg_agent_info"
				echo "inherit_gpg_agent_pid = $inherit_gpg_agent_pid"
...
startagent() {
	...
	if [ "$start_inherit_pid" = none ]; then
		...
		if [ "$start_prog" = ssh ]; then
			...
			# Revising gpg-agent start line
			# start_out=`gpg-agent --daemon --write-env-file $start_gpg_timeout 2>/dev/null`
			start_out=`gpg-agent --daemon 2>/dev/null`
			gpg_rc=$?
			if [ "$gpg_rc" != 0 ]; then
			    error "Error encountered starting gpg, may already be running, rc=$gpg_rc"
			fi
			#echo "Should have started gpg agent rc= $gpg_rc"
			#echo "begin: $start_out :end"
		else
